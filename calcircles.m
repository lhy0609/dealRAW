...
posX = 482;
posY = 646;% 计算圆心时窗口的中心位置
Length = 150;% 窗口大小
stX=posX-Length-1;
stY=posY-Length-1;

fin = fopen('3.raw','r');
I = fread(fin,'uint16=>uint16');% 读取raw文件

for i = 1:1:8 % 每一帧都有八幅图像，须看存时存了几帧
    if i<=8 
    ...
    It = uint8(It); % 将每个像素由uint16转为uint8，以便于显示
    zt = reshape(It,row,col);
    zt = zt';
    switch i % switch此处为了给每幅图像加镜像
        case 2 %522nm 左右翻转
            zt = fliplr(zt);
        case 3 %553nm 左右翻转
            %zt = fliplr(zt);
    ...
    stats = regionprops(Ipart2,'centroid','MajorAxisLength','MinorAxisLength','Eccentricity');% 关键函数
